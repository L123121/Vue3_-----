...
import { parseDecision } from './outputParser.js'
 ...
 */
async function requestDecision(providerPool, messages, signal, timeoutMs) {
    try {
        return parseDecision(await requestCompletion(providerPool, messages, signal, true, timeoutMs))
    } catch (error) {
        ...
        return parseDecision(await requestCompletion(providerPool, fallbackMessages, signal, false, timeoutMs))
    }
}
...
export async function runAgentLoop({
    ...
    for (let index = 0; index < MAX_AGENT_STEPS; index++) {
        ...
        const decision = await requestDecision(providerPool, messages, signal, remainingTime)
        ...
        try {
            ...
            const callIndex = steps.findIndex(s => s.id === callStep.id)
            steps[callIndex] = { ...callStep, status: 'error', result: message }
